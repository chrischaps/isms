...
| 2 🔶 | **SJ.5** prices from the players — **built 2026-09-25** (SESSIONS SJ.5; free gates green; the scripted town's graph in `docs/playtest/runs/sj5-town-scripted.md`); the paid seven-day run awaits approval | lever, agents | main | a day; ~$0.15 on approval | the card's tests ✅; `make e2e-agents` ✅; a seven-day sixteen-player run read on its own price graph ⏳ |
