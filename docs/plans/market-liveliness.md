...
| 2 🔶 | **SJ.5** prices from the players — **built 2026-09-25** (SESSIONS SJ.5; free gates green; the scripted town's graph in `docs/playtest/runs/sj5-town-scripted.md`); the paid week ran 2026-09-25 (`sj5-7day.md`, SESSIONS SJ.5b: clean, $0.11, every good but machines moved — all by the runner; the founders' foundries could not sell, fixed) — a re-run with the fix awaits approval | lever, agents | main | a day; ~$0.15 on approval | the card's tests ✅; `make e2e-agents` ✅; a seven-day sixteen-player run read on its own price graph ✅ once, ⏳ again with a player's ask on it |
