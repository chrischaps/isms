...
Phase 0b adds: `RequestStoreDraw` (Common Store, usually issued by the standing plan), `RequestStateStore { good, qty }`, `RequestTransfer { to_workplace }` and `DecideTransfer` (Directorate), `SetPlan { targets, materials_split, price_list, wage_grades }` (a typed `SetPolicy`), `ProposeAdmission`/`VoteAdmission` (coops), `SetShareRule` (coops), `FormUnion`/`CallStrike`/`OfferCollectiveAgreement`. Governance commands are Phase 2. S2.1 adds `Propose { title, text, kind }` (a human citizen; `NotInThisSociety` when the kind's tag is outside `proposal_kinds` or the citizen is outside `proposers`; householders and dormant citizens refused; at most `governance.open_proposals_per_citizen` open at once; a `PolicyChange` patch is validated against the constitution at `Propose`, and one touching `rationing` is a coordinator's to open (Q116); it closes at the end of the cycle it opens in) and `Vote { proposal, ballot: Yes | No | Abstain }` (one ballot per citizen, replaceable until the close; on an admission proposal it is the members' vote by the cooperative's rule). S2.2 adds `Stand { office }` and `Withdraw { office }` (a human citizen in the open election for an office the constitution declares; `NoElection` when none is open; `AlreadyExists` for a candidate already standing or a sitting holder whose seat is not on this ballot; `ConsecutiveTerm` where `OfficeSpec.consecutive` is false and the citizen's seat ends at this election's close or their last full term ended since it opened, Q118), `Approve { office, candidates }` (an approval ballot: any subset of the candidates, the empty set included, replaceable until the close; `NotACandidate` for a stranger), and the recall as `Propose { kind: Recall { office, citizen } }` (`NotAnOfficeHolder` when the citizen does not sit; one open recall per holder; carries by the office's `RecallRule` on the yes and no ballots, after the quorum). An `Election` proposal is refused: elections open by the calendar (Q125). S2.3 adds `OpenWorkplace { kind, slot? }` and `CloseWorkplace { workplace }` (a sitting Coordinator under `Governance::Direct`, else `NotAnOfficeHolder`, the code S2.2 chose and every power of an office reuses; opening spends `founding.materials` from the Common Store, `InsufficientGoods` when it is short, on a free slot of the kind; closing unassigns the workers that tick, returns the machines to the Store and frees the slot, Q120; only the collective's workplaces, `NotAuthorized` for another org's), opens `SetPlan { targets }` to a Coordinator under `Governance::Direct` (targets only: the four policy fields are `NotInThisSociety` there, the split being the assembly's vote; advisory, since no bonus is paid outside `Compensation::Scale` and no ratchet runs outside an administered policy; Q129), gives the `Honor { citizen }` proposal its effect (`UnknownCitizen` for a stranger, `AlreadyExists` for a second motion on one subject in a cycle, Q119; on passing, `Honored`), and refuses the System's `SetPolicy` under `Governance::Direct` unless the envelope's `client_kind` is `Sim` (`NotAuthorized`: the assembly changes policy by vote, the simulator keeps its stand-in; Q131). S2.4 gives the `Disbursement { org, to, asset }` proposal its effect as a member-owned org's vote (an association, cooperative or union: `NotInThisSociety` where no such org kind exists or the asset is money in a moneyless society; gated by the org kinds, not by `proposal_kinds`, and exempt from `proposers` and the title like an admission, Q133; `NotParty` for a non-member proposer or voter; `SelfDeal`, `InsufficientFunds`/`InsufficientGoods` and `PantryFull` checked at `Propose`; a majority of the membership either way closes it at once, a lone member's motion carries on the spot, and a split membership lapses at the cycle end, Q122; a carry the org can no longer cover closes failed, Q132), and refuses a manager's direct treasury or pantry `Transfer` on behalf of a member-owned org (`NotAuthorized`: the members disburse by vote).
...
| `EpochEnded { reason: Scheduled \| Collapse \| Operator, cycle, summary }` | tick / handle | `summary: EpochSummary` (S1.15, GDD §11.5): the last cycle's aggregates and every citizen's standing (net worth, self-made, ranked; the contribution record and a ranking by it where labor is by norm, E-2/Q157), frozen in the ending tick so the archive replays byte for byte; an operator's end carries the running cycle's figures |
...
| `ElectionOpened { office, seats, closes_cycle }`, `CandidacyDeclared { office, citizen }`, `CandidacyWithdrawn { office, citizen }`, `Approved { office, citizen, candidates }`, `ElectionClosed { office, approvals }`, `OfficeTaken { office, citizen, term_ends_cycle, approvals }`, `OfficeVacated { office, citizen, reason: TermEnded \| Absence \| Recalled }`, `ElectionRerun { office, seats, closes_cycle }`, `OfficeUnfilled { office, cycles }` | tick / handle | S2.2, appended after `Voted`. An election opens at epoch start for every office, and at 8j for any empty seat and for the seats whose terms end next cycle, closing at the next cycle end; `ElectionClosed` lists every candidate's approvals in rank order (most approvals, fewer past terms, lower id; Q118) and the `OfficeTaken` that follow seat the winners for `OfficeSpec.term_cycles`. A `CandidacyWithdrawn` drops the candidate from every approval ballot. `OfficeVacated` closes the citizen's term on `Offices.past_terms` (a full term only for `TermEnded`). `ElectionRerun` keeps an election nobody stood in open one more cycle, for the seats short by then. `OfficeUnfilled` is emitted once per cycle from `governance.unfilled_office_headline_cycles` on (GDD §8.3) |
| `WorkplaceOpened { workplace, org, kind, slot, materials_consumed, by }`, `WorkplaceClosed { workplace, org, slot, machines_returned, by }`, `Honored { citizen, proposal, cycle }` | handle / tick | S2.3, appended after `OfficeUnfilled`. `WorkplaceOpened` folds like `WorkplaceAdded` (the Materials leave the collective's stock, the Store in the Commune, and are consumed) and carries the coordinator who opened it; `WorkplaceClosed` follows the `Unassigned` of every worker, moves the machines from the workplace to the collective's stock, removes the workplace and frees its slot (Q120). `Honored` is emitted at 8j when an `Honor` proposal carries, one line on `Citizen.honors` that nothing removes (Q119); `EpochSummary.standings[].honors` counts them for the Commune's scoreboard |
| `Disbursed { proposal, org, to, asset, explain }` | handle | S2.4, appended after `Honored`. A member-owned org's disbursement vote carried: folds like `Transferred` (the org's treasury or pantry to `to`) and carries an `Explain` (`RuleId::Disbursement`: yes ballots and members). Emitted by the `Vote` that reaches the majority, or by the `Propose` of a lone member; a lapse at 8j is a `ProposalClosed { passed: false }` with the members' tally, as for an admission |
...
`presets/lexicon/<preset>.json` maps concept keys to display strings (GDD §15 table); `presets/copy/<preset>/` holds the Welcome Brief (Markdown with a few placeholders: `{{endowment}}`, `{{open_slots}}`, `{{norm_hours}}`, `{{rationing}}`) and the Chronicle headline templates (TOML: event kind → templates in the society's voice). A template key may be a variant keyed on a payload field, `Kind.path=value` (S2.9): `ProposalClosed.outcome=passed`, `OfficeVacated.reason=recalled`; `PolicyChanged` is narrated one line per field that moved, keyed `PolicyChanged.<field>` or `PolicyChanged.<field>=<new value>`. A `[words.<field>]` table gives enum values in the society's words for the Welcome Brief. `seed` refuses a preset whose copy directory is missing, so a society without a voice never exists. The neutrality checklist (GDD Appendix B) is applied to these files by review, not by code, but a test asserts every preset defines every lexicon key and every Chronicle-eligible event kind.
...
| Coordinator (Phase 2, S2.8) | `GET /s/{id}/plan/published` (`PublishedPlanView`: every workplace with its target beside yesterday's and today's output, workers and machines, the collective flag; every land slot and the unlimited kinds; the collective's id; the founding Materials against the Store's stock; the last `PlanPublished` this epoch from the log as cycle, tick and citizen; `i_coordinate`; 422 `NotInThisSociety` where nobody publishes a Plan, Q143), `PUT /s/{id}/offices/coordinator/plan` (`SetPlan`, targets only), `POST /s/{id}/workplaces` (`OpenWorkplace`), `DELETE /s/{id}/workplaces/{wid}` (`CloseWorkplace`); a non-holder's command is the engine's `NotAnOfficeHolder`, named; `WorkplaceView.target` and `last_cycle_output` so the Work screen reads the advisory target with no new route |
...
#### S2.2 — Offices: elections, terms, rotation, recall, vacancy *(engine, PR; done 2026-09-19)*
...
#### S2.3 — The coordinator's three powers and honors *(engine, PR; done 2026-09-20)*
...
#### S2.8 — The Coordinator workspace *(server + web; done 2026-09-20)*
...
#### S2.9 ∥ — The Commune's voice: lexicon, Welcome Brief, Chronicle *(copy + server; done 2026-09-21)*
...
| `governance.unfilled_office_headline_cycles` | 5 | S2.2, GDD §8.3 | cycles an office may stand short of holders before `OfficeUnfilled` says so each cycle |
| `sim.assembly_size` | 0 (6 in the Commune) | S2.4 | sim only: scripted humans `isms-sim` joins before epoch 0; two slates of three keep three seats filled with no consecutive terms |
| `sim.split_nudge` | 0.05 | S2.4 | sim only: how far each cycle's split proposal moves toward the scarcest sink |
| `sim.honor_every_cycles` | 10 | S2.4 | sim only: the assembly honors the top contributor every N cycles |
